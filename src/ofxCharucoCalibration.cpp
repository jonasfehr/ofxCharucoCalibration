...
bool ofxCharucoCalibration::add(cv::Mat img){
...
bool ofxCharucoCalibration::calibrate(){
    if(size() < 1) {
...
//    updateObjectPoints();
    ...
    double arucoRepErr;
    arucoRepErr = aruco::calibrateCameraAruco(allCornersConcatenated, allIdsConcatenated, markerCounterPerFrame, board, addedImageSize, cameraMatrix, distCoeffs, noArray(), noArray(), calibrationFlags);
        ...
        for(unsigned int frame = 0; frame < filteredImages.size(); frame++) {
            ...
            if(allIds[frame].size() > 0) {
                ...
                if(allCharucoCorners[frame].total() > 0) {
                    aruco::drawDetectedCornersCharuco( markersDetectedMat, allCharucoCorners[frame],
                                                      noArray(), Scalar(0,100,200)); //allCharucoIds[frame]
                }
            }
        }
    }
...
void ofxCharucoCalibration::updateReprojectionError(Mat perViewErrorsMat) {
...
std::vector<cv::Point3f> ofxCharucoCalibration::createObjectPoints(cv::Size patternSize, float squareSize, CalibrationPattern patternType) {
...
//int ofxCharucoCalibration::size(){
//    Calibration::size();
//}

bool ofxCharucoCalibration::clean(float minReprojectionError){
    int removed = 0;
...
void ofxCharucoCalibration::setupBoards(){
...
//bool ofxCharucoCalibration::findBoard(cv::Mat img, std::vector<cv::Point2f>& pointBuf, bool refine) {
//    vector< int > ids;
...
bool ofxCharucoCalibration::arucoMarkersDetectCollect(Mat img, Mat & imageOut){
    Mat image = img;
    vector< int > ids;
    vector< vector< Point2f > > corners, rejected;
...
bool ofxCharucoCalibration::arucoBoardDetect(Mat image, Mat & imageOut){
    
    Mat cameraMatrixLocal;
    Mat distCoeffsLocal;
    cameraMatrix.copyTo(cameraMatrixLocal);
    distCoeffs.copyTo(distCoeffsLocal);
    ...
    if(true)
        aruco::refineDetectedMarkers(image, board, markerCorners, ids, rejectedMarkers,
                                     cameraMatrixLocal, distCoeffsLocal);//, cameraMatrixLocal, distCoeffsLocal);
    ...
    if(ids.size() > 0)
        ...
        aruco::interpolateCornersCharuco(markerCorners, ids, image, charucoboard,
                                         charucoCorners, charucoIds, cameraMatrixLocal, distCoeffsLocal);
    
    // estimate charuco board pose
    bool validPose = false;
    if(cameraMatrix.total() != 0)
        validPose = aruco::estimatePoseCharucoBoard(charucoCorners, charucoIds, charucoboard,
                                                    cameraMatrixLocal, distCoeffsLocal, rvec, tvec);
...
bool ofxCharucoCalibration::calibrateCamera(cv::Mat img, Mat & imageOut) {
    //        cameraMatrix = Mat::eye(3, 3, CV_64F);
    ...
    // calibrate camera using charuco
    reprojectionError = aruco::calibrateCameraCharuco(allCharucoCorners, allCharucoIds, charucoboard, addedImageSize,
...
void ofxCharucoCalibration::setMarkerSize(float markerSize) {
    this->markerSize = markerSize;
}
float ofxCharucoCalibration::getMarkerSize() const {
    return markerSize;
}

void ofxCharucoCalibration::reset(){
    Calibration::reset();
    allCorners.clear();
    allIds.clear();
    allImgs.clear();
